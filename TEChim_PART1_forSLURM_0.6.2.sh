...
# Resources:
#SBATCH --time=0-23:59:00  # DAYS-HOURS:MINUTES:SECONDS
#SBATCH --ntasks=1
...
################################################################################
# RUN ON SLURM AS: sbatch --array=0-$(($(wc -l < sample.tsv)-1)) script.sh
################################################################################
# load modules
...
if [ -n "$SLURM_ARRAY_TASK_ID" ]; then
	...
	# Require exactly 4 tab-separated columns, even if one is empty
	if ! awk -F'\t' 'NF==4 { exit 0 } { exit 1 }' <<< "$line"; then
    	echo "ERROR: sample.tsv row $((SLURM_ARRAY_TASK_ID + 1)) must have exactly 4 tab-separated columns" >&2
    	echo "Got: [$line]" >&2
    	exit 1
	fi

	# Replace tabs with a non-whitespace separator so Bash preserves empty fields
	line=${line//$'\t'/$'\x1f'}

	IFS=$'\x1f' read -r FASTQ1 FASTQ2 SNo LNo <<< "$line"

	# Optional sanity checks
	if [[ -z "$FASTQ1" || -z "$SNo" || -z "$LNo" ]]; then
    	echo "ERROR: FASTQ1, SNo, and LNo must not be empty" >&2
    	exit 1
	fi
...
align_and_filter()
{
	...
	if [[ -z "${MaxFragLength:-}" ]]; then
    	if [[ -r ".${SNa}_maxfraglength_thissample" ]]; then
        	MaxFragLength=$(< ".${SNa}_maxfraglength_thissample")
    	else
        	echo "ERROR: missing .${SNa}_maxfraglength_thissample" >&2
        	exit 1
    	fi
	fi
	if [[ $stranded = "0" ]]; then
...
blast_on_longreads ()
{
    ...
    # Give GNU parallel a writable temp directory
	tmpdir="$wd/tmp/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
	mkdir -p "$tmpdir"
    
    # Step 3: parallel execution
    cat "${SNa}_S${SNo}_L${LNo}_out7_TExGENES_longreads.tsv" | \
        parallel --tmpdir "$tmpdir" --colsep ' ' -j "$nc" blast_single_read {1} {2} \
...
if [ -f "$FASTQ2" ]; then
	...
	create_fasta $FASTQ1
	MaxFragLength=$(awk 'NR%2==0 { if (length($0) > max) max = length($0) } END { print max+0 }' "${SNa}_S${SNo}_L${LNo}_LOOKUP.fa")
	echo "$MaxFragLength" > ".${SNa}_maxfraglength_thissample"
	echo "$MaxFragLength" >> "$wd/.${SNa}_maxfraglength"
