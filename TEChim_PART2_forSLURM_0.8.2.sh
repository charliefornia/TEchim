...
check_for_TE_splicesites()
{
	...
	do
		...
		if [ $a == "GENE-TE" ]
		then
			grep -F $b $REFpath$REFbase"_SPLICE_DONORS.bed" > "tmp."$SNa"_out14_ref_for_overlap.bed"
		...
		then
			grep -F $b $REFpath$REFbase"_SPLICE_ACCEPTORS.bed" > "tmp."$SNa"_out14_ref_for_overlap.bed"
			c=$(bedtools intersect -a "tmp."$SNa"_out13.bed" -b "tmp."$SNa"_out14_ref_for_overlap.bed" -loj -wa | head -n 1 | awk '{print $10}' | cut -d "|" -f2)
			rm "tmp."$SNa"_out14_ref_for_overlap.bed"
		else
			c="N/A"
...
{
        cd "$wd"
        echo " --> start tyding up TE breakpoints at ... $(date)" >> "$wd/$SNa_PART2_$logname.log"
        ...
        rm -f "$1"
        echo " <-- done tyding up TE breakpoints at ... $(date)" >> "$wd/$SNa_PART2_$logname.log"
}
...
add_splice_ratio()
{
	...
	# Get list of unique sample IDs (e.g. S1, S2, ...) from PART1 output directory names
	list_of_SNo=$(find "$path_to_PART1_output" -maxdepth 1 -name "${SNa}_S*" | rev | cut -d "/" -f 1 | awk '{gsub(/_/,"\t"); print $2}' | awk '!seen[$0]++' | rev)
	
	export SNa path_to_PART1_output stranded list_of_SNo

	# Define the processing function that operates on a single line of the input TSV
	process_chimera_line() {
		line="$1"
		splicesite=$(echo "$line" | awk '{print $12}')
		
		# Skip processing if the splice site is not a number (i.e., ".")
		if ! [[ "$splicesite" =~ ^[0-9]+$ ]]; then
			echo -e "$line"
			return
		fi

		# Extract key metadata fields from the input line
		chromosome=$(echo "$line" | awk '{print $2}')
		strand=$(echo "$line" | awk '{print $3}')
		fragtype=$(echo "$line" | awk '{print $6}')
		
		# Array to store per-sample (SNo) results in the format S1(42)|S2(17)...
		sums_by_SNo=()  # this will hold S1(42)|S2(37)|...
		
		# Iterate over each sample ID (SNo)
		for SNo in $list_of_SNo; do
			sum_for_SNo=0
		
			# Find all LNo subdirectories belonging to the current SNo
			list_of_LNo=$(find "$path_to_PART1_output" -maxdepth 1 -name "${SNa}_${SNo}_*" | rev | cut -d "/" -f 1 | awk '{gsub(/_/,"\t"); print $2"\t"$1}' | rev | grep "$SNo" | awk '{print $1}')
			
			# Iterate over each library (LNo) for the current sample
			for LNo in $list_of_LNo; do	
				SJout="$path_to_PART1_output${SNa}_${SNo}_${LNo}/${SNa}_${SNo}_${LNo}_STAR/${SNa}_${SNo}_${LNo}_out4_SJ.out.tab"
				
				# Depending on strand and fragment type, look in the correct column of SJ.out
				# Note that exon-intron junctions are sometimes off by +/- 1nt. We therefore pool all predicted junctions between -1 breakpoint +1
				if [[ "$strand" == "+" ]]; then
					if [[ "$fragtype" == "GENE-TE" ]]; then
						sum=$(awk -v chr="$chromosome" -v pos="$splicesite" '$1 == chr && ($2 >= pos - 1 && $2 <= pos + 1) {s += $7} END {print s+0}' "$SJout")
					elif [[ "$fragtype" == "TE-GENE" ]]; then
						sum=$(awk -v chr="$chromosome" -v pos="$splicesite" '$1 == chr && ($3 >= pos - 1 && $3 <= pos + 1) {s += $7} END {print s+0}' "$SJout")
					fi
				elif [[ "$strand" == "-" ]]; then
					if [[ "$fragtype" == "GENE-TE" ]]; then
						sum=$(awk -v chr="$chromosome" -v pos="$splicesite" '$1 == chr && ($3 >= pos - 1 && $3 <= pos + 1) {s += $7} END {print s+0}' "$SJout")
				...
				# Add to the running sum for this SNo		
				sum_for_SNo=$((sum_for_SNo + sum))
			done
		
			# Append result for this SNo
			sums_by_SNo+=("${SNo}(${sum_for_SNo})")
		done
		
		# Combine all SNo results into one pipe-separated field and append to line
		splice_summary=$(IFS="|"; echo "${sums_by_SNo[*]}")
		echo -e "$line\t$splice_summary"
	}

	# Export the processing function to be used by parallel
	export -f process_chimera_line

	# Run the function on each input line in parallel, preserving input order
	cat "$1" | parallel -j "$nc" -k process_chimera_line > "${SNa}_TE_chimericreads_final_withSpliceRatios.tsv"
...
if [ $stranded != "0" ]; then
	...
	add_splice_ratio $SNa"_TE_chimericreads_final.tsv"
else
