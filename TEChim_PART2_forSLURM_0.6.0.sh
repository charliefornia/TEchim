			...
			for SNo in $list_of_SNo
				...
				for LNo in $list_of_LNo
					do
						...
						if [[ -f $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam.bai" ]] ; then : ; else samtools index $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SN
o"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" ; fi
						...
						if [[ $(echo $line | awk '{print $3}') == "+" ]]; then
							if [[ $(echo $line | awk '{print $6}') == "GENE-TE" ]]; then
								...
								samtools view -f 65 -F 272 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v te="$(echo $line | awk '{print $5}')" '{ if($7 ~ te) {print $0}}
' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out3_TEreads"
								samtools view -f 97 -F 272 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v breakpoint="$breakpoint" -v geneend="$geneend" '{ if($7 == "=" &
& breakpoint<$8 && $8<geneend) {print $0}}' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out4_genereads"
							elif [[ $(echo $line | awk '{print $6}') == "TE-GENE" ]]; then
								endnt=$(($breakpoint+$MaxFragLength))
								region1=$(echo $chromosome":"$breakpoint"-"$endnt)
								samtools view -f 145 -F 256 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v te="$(echo $line | awk '{print $5}')" '{ if($7 ~ te) {print $0}
}' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out3_TEreads"
								samtools view -f 145 -F 288 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v breakpoint="$breakpoint" -v genestart="$genestart" '{ if($7 == 
"=" && genestart<$8 && $8<breakpoint) {print $0}}' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out4_genereads"
							fi
						elif [[ $(echo $line | awk '{print $3}') == "-" ]]; then 
							if [[ $(echo $line | awk '{print $6}') == "GENE-TE" ]]; then
								endnt=$(($breakpoint+$MaxFragLength))
								region1=$(echo $chromosome":"$breakpoint"-"$endnt)
								samtools view -f 81 -F 256 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v te="$(echo $line | awk '{print $5}')" '{ if($7 ~ te) {print $0}}
' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out3_TEreads"
								samtools view -f 81 -F 288 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v breakpoint="$breakpoint" -v genestart="$genestart" '{ if($7 == "
=" && genestart<$8 && $8<breakpoint) {print $0}}' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out4_genereads"
							elif [[ $(echo $line | awk '{print $6}') == "TE-GENE" ]]; then
								startnt=$(($breakpoint-$MaxFragLength))
								region1=$(echo $chromosome":"$startnt"-"$breakpoint)
								samtools view -f 129 -F 272 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v te="$(echo $line | awk '{print $5}')" '{ if($7 ~ te) {print $0}
}' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out3_TEreads"
								samtools view -f 161 -F 272 $path_to_PART1_output$SNa"_"$SNo"_"$LNo"/"$SNa"_"$SNo"_"$LNo"_STAR/"$SNa"_"$SNo"_"$LNo"_out4_Aligned.sortedByCoord.out.bam" $region1 | awk -v breakpoint="$breakpoint" -v geneend="$geneend" '{ if($7 == "=" 
&& breakpoint<$8 && $8<geneend) {print $0}}' | wc -l | awk '{print $1}' >> "tmp."$SNa"_"$SNo"_out4_genereads"
							fi
						fi
					done	
