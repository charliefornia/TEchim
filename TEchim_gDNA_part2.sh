...
do
	...
	cat "tmp."$SNa"_"$TE"_out07_TE-GENE.tsv" "tmp."$SNa"_"$TE"_out07_GENE-TE.tsv" | bedtools sort -i - > "tmp."$SNa"_"$TE"_out09.tsv"
	...
	# loop through list of genes and print hits in genes to output
	while read GENE
	do
		...
		awk -v g="$GENE" '{if ($12 ~ g) print g"\t"$1"\t"$6"\t"$9"\t"$4"\t"$11"\t"$10"\t"$13"\t"$7"\t"$8"\t""gDNA";}' "tmp."$SNa"_"$TE"_out09.tsv" >> $SNa"_out12_OUTPUT.tsv"
...
paste $SNa"_newcola" $SNa"_newcolb" > $SNa"_chimericreads_gDNA_final.tsv"
